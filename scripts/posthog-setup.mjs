...
const EVENT_DESCRIPTIONS = {
  ...
  hook_read: 'A whole-file read the hook decided on: outline, cached (re-read cache reminder) or raw, and why. Token and context buckets, extension, model family, agent class.',
  hook_reread: 'A file served as an outline or a cache reminder was read again (full, range, after-cache). The compression-harm signal.',
...
const PROPERTY_DESCRIPTIONS = {
  ...
  decision: 'hook_read: outline (compressed view served), cached (re-read cache reminder) or raw (file passed through).',
  agent_class: 'Who read: main (the conversation), readonly (Explore, Plan, …) or editing (any other subagent). Never a custom agent name.',
  cache_miss: 'hook_read: why the re-read cache did not apply to a file read before (missed, other-agent, changed, compacted, unknown).',
  turns_ago: 'hook_read decision=cached: main-thread turns since the file was first served.',
  kind: 'hook_reread: full, range or after-cache. hook_read: code, json or text.',
...
const DASHBOARDS = [
  ...
  },
  {
    ...
    insights: [
      ...
      { name: 'Cache hits vs re-reads', description: 'Re-read cache (1.0): reminders served vs files re-read anyway. The after-cache share is the cache\'s own harm rate.', query: trends({ series: [events('hook_read', { math: 'total', properties: [{ key: 'decision', value: ['cached'], operator: 'exact', type: 'event' }] }), events('hook_reread', { math: 'total', properties: [{ key: 'kind', value: ['after-cache'], operator: 'exact', type: 'event' }] })], interval: 'week', from: '-90d' }) },
      { name: 'Cache misses (why a known file was not cached)', query: trends({ series: [events('hook_read', { math: 'total', properties: [{ key: 'cache_miss', value: 'is_set', operator: 'is_set', type: 'event' }] })], display: 'ActionsBarValue', breakdown: 'cache_miss' }) },
      { name: 'Decisions by agent class', description: 'main vs readonly (Explore, Plan) vs editing subagents. Read-only agents get the lower 800-token bar.', query: trends({ series: [events('hook_read', { math: 'total' })], display: 'ActionsBarValue', breakdown: 'agent_class' }) },
      { name: 'Re-read rate by agent class (%)', query: trends({ series: [events('hook_read', { math: 'total', properties: [{ key: 'decision', value: ['outline', 'cached'], operator: 'exact', type: 'event' }] }), events('hook_reread', { math: 'total' })], formula: 'B / A * 100', interval: 'week', from: '-90d', breakdown: 'agent_class' }) },
