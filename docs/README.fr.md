...
> Vois ce que tes sessions Claude Code coûtent vraiment, et coupe la part qui compte — sans changer ta façon de coder.
...
GitHub exclut les pré-releases de `/releases/latest` : les installeurs et `cork-ai update` suivent donc les versions stables, sauf demande explicite :
...
cork-ai update --pre      # déjà installé : passe sur le canal pre et met à jour (--stable pour revenir)
```

Le choix est mémorisé (`config set channel pre|stable`). Après `gain`, `context`, `doctor`, `hooks`, `report` ou `models`, une ligne te prévient quand ton canal a une version plus récente ; une install stable à jour qui pourrait essayer une release candidate reçoit une invitation plus discrète, au plus une fois tous les trois jours.
...
cork-ai hooks status    # lesquels des 7 hooks sont actifs
...
| `PostToolUse` **Edit / Write** | Suivi des fichiers édités, garde contexte |
| `PostToolUseFailure` **Edit / Write** | Edit échoués sur fichiers outlinés : le fichier est servi brut pour de bon (Claude Code ≥ 2.1.119) |
...
```bash
cork-ai update            # remplace le binaire par la dernière release (--check pour seulement regarder, --pre pour les release candidates)
cork-ai config set updateCheck false  # pas de vérification quotidienne, pas de ligne de rappel après les commandes
...
```bash
cork-ai gain              # la dernière session vue par cork-ai : ses outlines (en cours ou terminée) et son digest SessionEnd
...
- **OS** : Linux x64 / arm64 sur glibc (Ubuntu 20.04+, Debian, Fedora… ; pas encore de binaire Alpine/musl), macOS (Intel + Apple Silicon), Windows (natif + WSL2)
