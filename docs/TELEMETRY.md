...
| `uninstall` | `cork-ai hooks remove` / `uninstall` | `days_installed` (a **duration**, never the date it started), `sessions` and `saved_tokens` over the install's lifetime — all three null when the machine cannot know. Leaving is the measurement that keeps the rest honest: without it, an uninstall looks exactly like a holiday, and cork-ai cannot tell when it disappoints |
